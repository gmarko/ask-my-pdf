...
def on_api_key_change():
    ...
    # TODO: refactor user 'calculation' from get_storage
...
def ui_api_key():
    ...
    if api_key:
        st.write('## 1. OpenAi Api Key cargada')
    else:
        st.write('## 1. OpenAI API key not found in environment variables')
...
def ui_pdf_file():
    st.write('## 2. Sube tu PDF')
    
    t1, t2 = st.tabs(['UPLOAD', 'SELECT'])
    with t1:
        st.file_uploader('pdf file', type='pdf', key='pdf_file',
    ...
    with t2:
        ...
        st.selectbox('select file', filenames, on_change=on_change,
                     key='selected_file')
        b_delete()
...
def ui_fragments():
    ...
    st.selectbox('fragment size (characters)', [
                 0, 200, 300, 400, 500, 600, 700, 800, 900, 1000], index=3, key='frag_size')
    b_reindex()
    st.number_input('max fragments', 1, 10, 4, key='max_frags')
    st.number_input('fragments before', 0, 3, 1,
                    key='n_frag_before')  # TODO: pass to model
    st.number_input('fragments after',  0, 3, 1,
                    key='n_frag_after')  # TODO: pass to model
...
def ui_model():
    ...
    st.selectbox('embedding model', [
                 'text-embedding-ada-002'], key='model_embed')
...
def ui_question():
    st.write('## 3. Pregunta sobre el contenido del Pdf' +
             (f' to {ss["filename"]}' if ss.get('filename') else ''))
    disabled = False
    st.text_area('question', key='question', height=100, placeholder='Pregunta sobre tu pdf',
...
def b_save():
    ...
    if st.button('save encrypted index in ask-my-pdf'):
